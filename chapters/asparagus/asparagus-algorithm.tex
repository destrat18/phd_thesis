\section{Automated Gas Analysis Framework}\label{sec:asparagus-algorithm} 
...
\begin{example}
	Figure~\ref{fig:asparagus-simple} provides a simplified PTS representation for each of our example contracts of Figure~\ref{fig:asparagus-runex}. In practice, our tool automatically converts the CFG provided by~\cite{albert2018ethir} to an RBR. See Section~\ref{sec:asparagus-algorithm-synthesis} for details. However, the resulting PTSs are usually not human-readable, so Figure~\ref{fig:asparagus-simple} is not a direct output of our tool, but a simplified version.  
\end{example}
...
\para{Invariant Generation} Invariant generation and more specifically, the automated synthesis of linear/polynomial inductive invariants, is an orthogonal and well-studied problem with practically efficient tools such as~\cite{colo2003linear, chatterjee2020polynomial}. As such, in the sequel, we assume that every PTS comes with invariants generated using one of these tools. 
