\section{Experimental Results}{\label{sec:mining-ethereum-experiments}}

\paragraph{Implementation}
We implemented our approach in Python 3 and Rust. Our tool is available at~\url{https://anonymous.4open.science/r/boosting_gas_revenues_of_ethereum_miners/}. We used Anvil~\cite{anvil} and Erigon~\cite{erigon} to execute Ethereum transactions in our samples and obtain their gas usage. Moreover, we used  Linfa~\cite{linfa} to generate decision trees and relied on SCIP~\cite{BolusaniEtal2024OO} for solving Integer Linear Programming (ILP) instances.
