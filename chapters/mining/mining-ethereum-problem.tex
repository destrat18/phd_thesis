\section{A Randomized Framework for Ethereum Block Construction}\label{sec:mining-ethereum-problem} 
...
(formal definitions and details can be found in Section~\ref{sec:mining-ethereum-algorithm}).
...
contract ToyGasDependency {
	...
	function g() public {
		...
		\end{lstlisting}
		\caption{A toy contract illustrating gas dependency.}
		\label{fig:mining-ethereum-toy-gas-dependency}
	\end{subfigure}%
	\hfill
	\begin{subfigure}[t]{0.48\textwidth}
		\begin{lstlisting}[language=Solidity, numbers=none]
contract IndirectGasDependency {
	uint n = 0, m = 0;
...
\paragraph{MEV~\cite{foundation2025maximal}} The term \emph{Miner Extractable Value} or \emph{Maximal Extractable Value} (MEV) was popularized by~\cite{daian2020flash} to describe the maximum profit a dishonest miner can obtain by exploiting their knowledge of transactions. This is often called ``toxic'' MEV in the blockchain community. For instance, miners can engage in front-running, back-running, and sandwich attacks, leveraging their ability to manipulate, reorder, or censor transactions~\cite{zhou2021high, luu2016making, kushwaha2022ethereum}. Such attacks can even threaten the security of Ethereum's consensus layer~\cite{daian2020flash}. Several countermeasures against transaction reordering have been proposed~\cite{heimbach2022sok}, and formal verification tools exist to identify profitable MEV opportunities~\cite{babel2023clockwork, judmayer2022estimating}.
