...
% \paragraph{Network and Transaction Lifecycle} TODO: Describe P2P network, mempool, propagation, confirmations, and reorgs.
% \paragraph{Reward Mechanisms} TODO: Compare block rewards and transaction fees (and fee burning where relevant).
% \paragraph{UTXO vs.~Account Model} TODO: Contrast UTXO vs account-based models; mention EUTXO.

We next briefly review Bitcoin, Cardano, and Ethereum, emphasizing aspects relevant to this thesis.
...
This section provides a quick introduction to Ethereum and its gas model, following~\cite{wood2014ethereum,beigepaper}.
...
\para{Gas Usage Formula} The Ethereum protocol fixes a specific gas cost for every one of the EVM opcodes using a complicated formula to address the different computational workloads of different opcodes. See~\cite[Page 14]{beigepaper} for a complete table of gas costs. We follow these costs in our implementation but their specific values have no bearing on the theory. In summary:
\begin{itemize}
